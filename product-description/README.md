...
- **Where this repo lives.** This description lives in `product-description/` on a branch of the 3D Chess repository itself, because the working environment could keep nothing else. Work on the description never changes `client/` or `server/`; every document cites the source commit it describes, `90142a3` (fixes to the product land separately, and the documents are then brought up to the new commit).
...
The documents were first written against commit `d94507b` and have since been brought up to `90142a3`, which carries the fixes for B-01 to B-09 (see [`bug-triage.md`](bug-triage.md), each entry's **Fix** line). The checklists were rewritten to match, and their Result columns come from a scripted pass against `90142a3`; no document is marked `verified`, which still needs a person's pass.
...
The source of truth is the 3D Chess repository (`howard36/3d-chess`), the parent directory of this one, at commit `90142a3`. The relevant locations are:
