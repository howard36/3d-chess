...
The player has just clicked "Start New Game" (see [creating a game](creating-a-game.md)). On the same dark page, under the title "3D Chess", the page now reads "Game created! Share this link with a friend:" above the share link, `{origin}/game/{id}` (for example `https://…/game/K7Q2ZD`), in large bold type in a dark gray box. The link is plain text: it is not clickable, there is no copy button, and nothing on the page responds to a click. The address in the browser's address bar is the same link, so the player can copy it from there, or select the text in the box and copy that.
...
At the instant the screen appears, nothing is decided that the player can see. The creator's color was fixed by the server when the game was created and is held in this browser as the stored seat, but the screen does not show it.

### End without sending

Most of the wait is spent here: nobody has joined yet. The screen has no controls, so nothing the creator does can end the wait or send anything, and nothing the creator does is recorded. There is no timeout: the page waits for as long as it stays open, and the game waits on the server whether or not the page is open.
...
- it records the second seat, whichever color the creator did not get. The game is now full for good: any later join, including the creator's own from another browser, is refused with "Game full";
...
Nothing the creator can do changes what happens when someone joins: the color was fixed at creation, and the screen takes no input.
...
**Keyboard, touch, and screen size.** There is nothing to operate. The link can be selected and copied with the mouse or a long press on a touch screen; there is no copy button, and from the keyboard alone the practical source is the address bar, which holds the same link. The screen is a single centered column; see [screen sizes and touch](../cross-cutting/screen-sizes-and-touch.md) and the open questions for how the link fits a narrow window.
...
- **A creator whose browser does not store the seat.** With storage disabled, the page finds no stored seat and shows the creator the join screen instead of this one; clicking "Join Game" there is refused with "Already in a game". The board still appears when an opponent joins, but before that a drop or a reload loses the seat. See [creating a game](creating-a-game.md#edge-cases).
...
- The share link is one long unbroken piece of text in large type, and the page cannot scroll. Whether it wraps, or runs off both edges of a narrow window, was not checked.
- Back, then Forward before the start screen's fresh connection opens, sends two rejoins: the rejoin is queued on arrival because the page still sees the old connection's session number (a reset does not clear it, `client/src/hooks/useGameSocket.ts:89-98`), and it is sent again when the new connection opens, before the first answer has arrived (`client/src/screens/GameScreen.tsx:95-101`). The second is refused with "Already in a game", which appears in the error banner. Read from code; not reproduced. The same would happen when Forward is pressed while the start screen is still reconnecting after an outage.
