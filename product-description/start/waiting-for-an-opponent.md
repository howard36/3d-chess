...
- **Back, then Forward, quickly.** Going Back to the start screen and Forward again before the start screen's fresh connection has opened sends one rejoin, when that connection opens, and the share-link screen comes back without an error. One narrower timing still sends two: if the start screen's connection opened and then dropped, and the player goes Forward while it is reconnecting, the page asks for its rejoin at once, and when a connection opens it sends that request and then another; the second is refused, and "Error: Already in a game" appears in the error banner. The seat is not affected. This looks like a bug; see open questions.
...
- "Copy link" is absent on a plain `http://` address other than `localhost`, because the page shows it only when the browser offers the clipboard (`client/src/screens/GameScreen.tsx:503`). That affects only development and testing on another device; production is served over `https://`. Read from code; not tried on a device.
- Back, then Forward while the start screen's connection is reconnecting after having opened, sends two rejoins (read from code; not reproduced). A return to the start screen sets the session number to 0 (`client/src/hooks/useGameSocket.ts:107`), and the game page waits for a nonzero one before rejoining (`client/src/screens/GameScreen.tsx:127`), so the common case, Forward before the fresh connection has opened, sends one rejoin. But once that connection has opened, its number stays nonzero after it drops; the arriving page then asks for a rejoin at once, which is queued (`useGameSocket.ts:80-89`), and when a connection opens the queue is sent (`:132-136`) and the page sends its own rejoin as well, because no answer has arrived yet (`GameScreen.tsx:128-139`). The server refuses the second with "Already in a game" (`server/modal_app.py:402`, `:290-292`), which appears in the error banner. See [bug triage](../bug-triage.md) B-10.
