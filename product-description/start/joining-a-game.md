...
If the connection drops now, before the seat confirmation arrives, the answer is lost with it. The page keeps showing "Joined game, waiting for start...", the amber "Reconnecting…" box appears at the top right, and the browser retries on its [retry schedule](../glossary.md#the-connection). When a connection opens, the page sends the join again on it, with the same client id, and goes on doing so on every new connection until an answer arrives. The server answers the repeat like any join: if the first join had claimed a seat, this tab gets that same seat back; if the first join never arrived, the repeat is the first the server sees, and takes the free seat or is refused like any other.
...
**The opponent.** The creator's share-link screen is replaced by the board at the same moment, and the creator is told "Opponent: online"; see [waiting for an opponent](waiting-for-an-opponent.md). The joiner is told whether the creator is connected. Neither ever sees the other's join screen or "Joined game, waiting for start...". A join re-sent after a drop reaches the creator as another start notice, which changes nothing on the board, and another "Opponent: online"; if the server had already noticed the lost connection, the creator saw "Opponent: offline" in between.
