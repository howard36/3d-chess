...
stateDiagram-v2
    ...
    flight --> replaced : repeated rejoin finds another tab holding the seat
...
The rejoin leaves the browser as soon as the page's connection is open: at once if it already is, otherwise the moment it opens. It names the game id from the address, the stored color, and this tab's [client id](../glossary.md#requests), and it [takes over](../glossary.md#the-connection) the seat: a fresh page is the player choosing this tab (a history jump from another game's page is the exception; see the edge cases). Nothing on screen marks this moment.

> Technical note: The page waits for a connection that has opened since the page's connection was last started or reset, and sends the rejoin on it directly rather than through the connection's queue. That covers a page loaded from scratch, a history jump between two game pages, and a return from the start screen before that screen's connection has opened. Only a return from the start screen while that screen's connection is reconnecting, after having opened once, goes through the queue, and that path sends the rejoin twice; see the [edge cases](#edge-cases).
...
Only the page's first rejoin takes over. If its answer is lost to a drop, the rejoin the next connection sends does not: it still replaces this tab's own lingering connection, but if another tab of this browser holds the seat by then, the server answers with [seat in use](../glossary.md#the-connection), and this page shows the replaced dialog instead of taking the seat. See [connection loss](connection-loss.md).
...
"Cannot rejoin" means the game is gone, almost always because it [expired](../glossary.md#games-and-seats) about 30 days after it was last active. Clicking "Join Game" on the join screen that follows is refused with "Error: Cannot join". "No such seat to rejoin" means the game exists but nobody holds the stored color. Because the browser stores only seats the server assigned, and a seat is never given up, a real player meets it only if the stored seat was edited by hand, an expired game's id was later drawn again for a new game, or the stored seat was overwritten by a history jump (see the [edge cases](#edge-cases)).

"Already in a game" should never answer a rejoin, since the rejoin is the first request on a fresh connection. It does in one timing case described in the [edge cases](#edge-cases); it is shown in the banner and changes nothing else. "Seat in use" answers only a repeated rejoin that does not take over, and shows as the replaced dialog, not in the banner. Every text is catalogued in [error messages](../cross-cutting/error-messages.md).
...
| Connection state | A page loaded from scratch always starts connecting; if the first attempt fails, "Reconnecting…" appears over the share-link screen and the rejoin waits for the retry schedule. From the start screen the connection is usually already open and the rejoin goes out at once; if it has not opened yet, the rejoin waits for it. If it opened and has since dropped, the rejoin is sent twice (see the edge cases). Replaced cannot be the state at the start: every return has a fresh connection. | A drop loses the answer, and the next connection sends the rejoin again, without taking over. Replaced: the replaced dialog appears over whichever screen is showing. |
...
| The connection drops | "Reconnecting…" appears over the share-link screen; the retry schedule runs; the rejoin goes out when a connection opens. | The answer is lost. The next connection sends the rejoin again by itself, without taking over: it succeeds unless another tab of this browser took the seat in the meantime, in which case the replaced dialog appears. If the server handled the lost rejoin, the opponent saw the player come online and go offline again. |
...
**Connection.** Every return is a new connection as far as the server is concerned, even when it reuses the start screen's: the server knows nothing about the tab until the rejoin arrives. One rejoin is sent per new connection; the first takes over the seat, and if the connection drops before the answer, the next one sends another that does not. The reloaded page's old connection either closes as the page unloads or lingers until the rejoin replaces it; either way the new page is unaffected.
...
**Stored seat.** Read once when the page loads; it decides between rejoining and the join screen. A rejoin never writes it, because a snapshot returns a seat rather than assigning one. It is deleted only by a refusal before the page has shown the game, as set out under [the answer arrives](#the-answer-arrives). It has no expiry of its own: the browser keeps one entry per game ever played in it, and nothing lists or removes them except that refusal and clearing the browser's site data. One path writes it wrongly: see "Jumping from one game page to another" in the edge cases.
...
- **Coming back into a game page while the start screen is still connecting.** Browser Back or Forward from the start screen onto a game page, while that screen still shows "Connecting to server…", sends one rejoin, when the connection opens. The one timing that still sends two is a start screen whose connection opened and then dropped: coming back while it shows "Reconnecting to server…", the page asks for its rejoin at once, and when a connection opens it sends that request and then another. The server answers the first with the snapshot and refuses the second with "Already in a game". The game appears normally, with "Error: Already in a game" in the banner until it is dismissed. This looks like a bug; see open questions.
- **Jumping from one game page to another.** The browser's history menu (a long press or right-click on Back or Forward) can move from one game page straight to another without passing through the start screen. The page resets the connection as it arrives, as the start screen would: the previous game's opponent sees "Opponent: offline", and the new page starts afresh and rejoins the game in its address with that game's stored seat, or shows the join screen if there is none. That rejoin does not take over (unless the page being left had never held a seat), so if another tab of this browser holds that seat, this page shows the replaced dialog. One case goes wrong: if the page being left had seen its own game start, or its own join confirmed, live on that page (rather than only through a snapshot), the new page writes the previous game's color as its own game's stored seat before the reset reaches it. When the two colors differ, or the new game had no stored seat, the page then rejoins as the wrong color: that seat is usually the opponent's, so the page shows the replaced dialog while the opponent is connected ("Play here" would take the opponent's seat), and takes the opponent's seat outright while the opponent is away. Reloading does not repair it: the wrong color is now the stored seat, and a reload's rejoin takes over, so it takes the opponent's seat even while the opponent is connected. In a game still waiting for an opponent, the rejoin is refused with "No such seat to rejoin", the wrong stored seat is deleted, and the page falls back to the join screen, where "Join Game" gets the player's own seat back if this tab created the game. This looks like a bug; see open questions.
...
- **History jump writes the wrong stored seat (suspected bug).** The connection is reset on a change of game id, in a layout effect (`client/src/App.tsx:26-39`), and each game id gets a fresh page (`App.tsx:12-15`). But the new page renders once, and runs its effects, with the previous game's messages before the reset clears them. Its effect that saves an assigned seat (`client/src/screens/GameScreen.tsx:143-149`) then finds the previous page's seat confirmation or start notice (`client/src/game/session.ts:48`) and writes that color under the new game's id; its effect that marks the seat as held (`GameScreen.tsx:118-120`) also turns off taking over. After the reset, the page rejoins the new game with the previous game's color and `takeover: false` (`GameScreen.tsx:126-139`). Reproduced in a jsdom harness with a stand-in connection (the new game's stored seat overwritten; the rejoin sent with the previous color, not taking over); not tried through a real browser's history menu. Harmless when the colors match, apart from not taking over.
- **Duplicate rejoin (suspected bug, narrower than before).** A return to the start screen sets the session number to 0 (`client/src/hooks/useGameSocket.ts:107`) and the game page waits for a nonzero one (`GameScreen.tsx:127`), so a return before the start screen's connection has opened sends one rejoin. If that connection opened and then dropped, its number stays nonzero; the arriving page's rejoin is queued (`useGameSocket.ts:80-89`), sent when a connection opens (`:132-136`), and followed by the page's own rejoin for the new connection, since no answer has arrived yet (`GameScreen.tsx:128-139`). The server refuses the second (`server/modal_app.py:402`, `:290-292`). Read from code; not reproduced. See [bug triage](../bug-triage.md) B-10.
- **Joined screen skipped after "Cannot rejoin" (suspected bug, minor).** The join-failure check looks at every error the page has ever received (`GameScreen.tsx:154-162`; [bug triage](../bug-triage.md) B-15), so the earlier "Cannot rejoin" ends the join the moment "Join Game" is clicked, before the server has answered. The join is refused anyway on an expired game, but the button stays clickable and each click sends another join. The same check means a second click after "Game full" also skips the joined screen; that belongs to [joining a game](../start/joining-a-game.md).
- The stale-seat check (`GameScreen.tsx:166-177`) also looks at every error received rather than the rejoin's own answer. Harmless in practice, for the reason in the technical note under [the answer arrives](#the-answer-arrives).
