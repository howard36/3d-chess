...
The rejoin leaves the browser as soon as the page's connection is open: at once if it already is, otherwise the moment it opens. It names the game id from the address and the stored color. Nothing on screen marks this moment.

> Technical note: On a page loaded from scratch, the rejoin is not put in the connection's queue; the page waits for the connection's first opening and sends it then, which looks the same to the player. Only a return from the start screen while that screen's connection is still opening goes through the queue, and that path sends the rejoin twice; see the [edge cases](#edge-cases).
...
From the moment the page loads until the answer arrives, a page with a stored seat shows the [share-link screen](../start/waiting-for-an-opponent.md): the title "3D Chess", "Game created! Share this link with a friend:", and the link in a dark box. It shows it to every returning player. For the creator of a game nobody has joined it is the right screen. For a joiner, for either player of a game in progress, and for either player of a finished game it is the wrong one, and on an ordinary connection it is a flash, gone before it can be read.
...
Nothing on the screen takes input. There is no board and no "Join Game" button; the link is plain text that can be selected and copied. The only other things that can appear are the [error banner](../game-page/error-banner.md), for a refusal, and the replaced dialog, if another tab of this browser rejoins the same game after this one.
...
- the position is replayed from the whole record and drawn in place. No move in the record glides, not even the latest ([the view](../foundations/the-view.md#motion)); the teal last-move trace marks the latest move's two cells, and a King in check already glows red;
...
- [the board takes input](../foundations/input-model.md#when-the-board-takes-input) at once. On the player's turn their pieces can be selected immediately ([making a move](../play/making-a-move.md)); on the opponent's turn nothing can be selected ([the opponent's move](../play/the-opponents-move.md));
...
- no snapshot of any kind has arrived on this page since it loaded (or since it last came back from the start screen), not even one saying the game has not started;
...
"Already in a game" should never answer a rejoin, since the rejoin is the first request on a fresh connection. It does in one timing case described in the [edge cases](#edge-cases); it is shown in the banner and changes nothing else. Every text is catalogued in [error messages](../cross-cutting/error-messages.md).
...
| How you reached the page | Returning with a stored seat: this document, whatever the route. A visitor without one: the join screen, nothing sent. The creator arriving from "Start New Game" and the joiner right after "Join Game" already hold the seat and do not rejoin. Any route that loads the page from scratch starts a new connection; Back or Forward from the start screen reuses that screen's. | Leaving while in flight: see "Leaving the game page within the app" below. |
| Connection state | A page loaded from scratch always starts connecting; if the first attempt fails, "Reconnecting…" appears over the share-link screen and the rejoin waits for the retry schedule. From the start screen the connection is usually already open and the rejoin goes out at once; if it is still connecting or reconnecting, the rejoin is sent twice (see the edge cases). Replaced cannot be the state at the start: every return has a fresh connection. | A drop loses the answer, and the next connection sends the rejoin again. Replaced: the replaced dialog appears over whichever screen is showing. |
| Game state | Unknown until the snapshot. Not started: the share-link screen stays. In progress: the board screen. In check: the King already glows red. Over: the end-game dialog at once. Frozen: the frozen-board banner. Expired: the refusal and the join screen. | The opponent's move arriving right after the snapshot can put the player in check or end the game. |
...
| Input device | The keyboard shortcuts, the browser's buttons, a mouse's back and forward buttons, and a mobile browser's own reload and back controls all do the same thing. Nothing on the share-link screen takes a click, a tap, or a key. | No effect. |
...
| Pressing elsewhere or turning the view | There is no board yet. Clicks on the share-link screen do nothing; the link can be selected and copied as text. | Same. |
| Leaving the game page within the app | Nothing has been sent, so nothing happens on the server; the start screen appears as usual. "Start new game" does not exist yet (the end-game dialog comes only with the answer), and "Back to start" exists only on the crash screen, which loads the start screen from scratch. | The server may already have handled the rejoin: it tied the connection to the seat and told the opponent the player was online. Arriving at the start screen resets the connection, so the opponent then sees "Opponent: offline". The answer is ignored. The stored seat is kept. |
...
| The connection drops | "Reconnecting…" appears over the share-link screen; the retry schedule runs; the rejoin goes out when a connection opens. | The answer is lost. The next connection sends the rejoin again by itself, so a return never gets stuck waiting the way a create or a join can. If the server handled the lost rejoin, the opponent saw the player come online and go offline again. |
...
**Connection.** Every return is a new connection as far as the server is concerned, even when it reuses the start screen's: the server knows nothing about the tab until the rejoin arrives. One rejoin is sent per new connection, and if the connection drops before the answer, the next one sends another. The reloaded page's old connection either closes as the page unloads or lingers until the rejoin replaces it; either way the new page is unaffected.

**The opponent.** Sees "Opponent: offline" when the player's old connection closes (a reload, a closed tab, a return to the start screen) and "Opponent: online" when the rejoin is handled; an old connection that lingered and was replaced never shows as offline. Nothing else reaches them: their board, turn, and selection are untouched, and they can go on moving while the player is away. Their moves are recorded and are in the player's snapshot. See [presence](../foundations/connection-and-seat.md#presence).

**Other tabs and devices.** The stored seat is shared by every tab and window of the same browser profile, so a return in any of them rejoins as the player, and the newest rejoin takes the seat: opening the game in a second tab puts the first behind the replaced dialog ([a second tab](second-tab.md)). Another browser, another profile, another device, or a private window has no stored seat for the game: the link shows the join screen there, and "Join Game" is refused with "Game full" once both seats are taken. A private window's own stored seats survive reloads in that window and are forgotten when the private session ends. With storage disabled nothing is ever stored, so every return is a visitor's.

**Game over.** A finished game can be reopened for as long as it exists. Its snapshot is still a snapshot, so the stored seat is kept, and every return shows the end-game dialog over the final position at once, with the teal trace on the final move and no glide. The only way on is "Start new game". Once the game expires, the next return is refused and the stored seat is deleted.
...
**Keyboard, touch, and screen size.** Reload, Back, and Forward work from the keyboard as usual, and a mobile browser's own controls work the same way. The share-link screen has nothing to focus or press, and the board screen that follows starts at the default view at any window size; see [screen sizes and touch](../cross-cutting/screen-sizes-and-touch.md).
...
- **Coming back into a game page while the start screen is still connecting.** Browser Back or Forward from the start screen onto a game page, while that screen still shows "Connecting to server…" or "Reconnecting to server…", sends the rejoin twice on the new connection: once as the connection opens and once more as the page notices the new connection. The server answers the first with the snapshot and refuses the second with "Already in a game". The game appears normally, with "Error: Already in a game" in the banner until it is dismissed. The easiest way to meet it is "Start new game" in the end-game dialog followed at once by Back, which leads straight to the finished game; a cold or unreachable server widens the window. This looks like a bug; see open questions.
- **Jumping from one game page to another.** The browser's history menu (a long press or right-click on Back or Forward) can move from one game page straight to another without passing through the start screen. The page then keeps the previous game's connection and everything the server said about it: the address shows the new game, the screen still shows the old one, and the new game is never rejoined. Moves made there are played in the old game. Reloading fixes it. This looks like a bug; see open questions.
...
- **A refusal after the page has shown the game.** A rejoin after a drop could in principle be refused too, if the game vanished while the page was open (it expired during a very long outage). The page has already shown the game, so the stored seat is kept: the page stays on the screen it had, with the error in the banner, and any move is refused with "Not in a game" because the connection holds no seat. Reloading then deletes the stored seat as described above.
...
- **Duplicate rejoin (suspected bug).** Coming back into a game page from the start screen while that screen's connection is still opening sends two rejoins, and the second is answered "Error: Already in a game". The page sends a rejoin whenever it sees a connection it has not yet rejoined on (`client/src/screens/GameScreen.tsx:95-101`). The start screen's reset (`client/src/hooks/useGameSocket.ts:89-98`) does not change the connection's id, which changes only when a connection opens (`useGameSocket.ts:115`), so the arriving page queues a rejoin against the old id (`useGameSocket.ts:76-80`) and then sends another when the new connection opens with a new id. The server refuses a second rejoin on one connection (`server/modal_app.py:341`, `260-262`). Reproduced in a jsdom harness using the real connection hook and a mock server (two rejoins on the new connection); the banner was not seen in a real browser.
- **History jump between two game pages (suspected bug).** The connection is reset only on arrival at `/` (`client/src/App.tsx:18-20`). When the game id changes in place, the page re-reads the stored seat but keeps the old game's messages (`GameScreen.tsx:46-50`), and the rejoin check then finds the old game's seat on the connection and sends nothing (`GameScreen.tsx:98`). Reproduced in the same harness (no rejoin sent; the previous game's seat label still shown); not tried through a real browser's history menu.
- **Joined screen skipped after "Cannot rejoin" (suspected bug, minor).** The join-failure check looks at every error the page has ever received (`GameScreen.tsx:116-124`), so the earlier "Cannot rejoin" ends the join the moment "Join Game" is clicked, before the server has answered. The join is refused anyway on an expired game, but the button stays clickable and each click sends another join. Reproduced in the harness. The same check means a second click after "Game full" also skips the joined screen; that belongs to [joining a game](../start/joining-a-game.md).
- The stale-seat check (`GameScreen.tsx:126-139`) also looks at every error received rather than the rejoin's own answer. Harmless in practice, for the reason in the technical note under [the answer arrives](#the-answer-arrives).
...
- Whether a rejoin, which only reads the game (`server/modal_app.py:150-157`), counts as activity for expiry is not known.
