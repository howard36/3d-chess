...
On White's board nothing moves. There is no glide, the teal [last-move trace](../glossary.md#selection-and-board-state) stays on White's last move, and the turn indicator still reads "Black to move". A red box appears just below the turn indicator: "Move 12 in this game's history is not a legal move for this client (likely an app version mismatch). The board is frozen at the position before it." The move list gains Black's move as the second half of row 6, exactly as the server recorded it. Nothing of White's can be selected, and nothing ever will be again: the server is waiting for White's move, and White's board does not take input.
...
- **A different version of the app.** The app and the server are deployed separately (the top-level README, "Development": the server from CI, the app through Cloudflare Pages), and a tab keeps running the version it loaded until it is reloaded; reconnecting does not reload the app. Nothing compares versions, between the two players or with the server. If one version changes a rule, a move it allows may be one the other cannot replay. This is what the banner's "(likely an app version mismatch)" refers to. Two players who loaded the same version agree on every move either of them can offer, with the rare exceptions below.
- **The app itself, rarely.** Two paths, read from code and not reproduced:
  - *The moment after a reconnect.* When a new connection opens, the board takes input again at once, against the position it showed before the drop, and the rejoin's [snapshot](../glossary.md#requests) arrives about one round trip later. If the player's own move had been recorded but its echo was lost in the drop, and the opponent answered while the page was disconnected, the server is back at the player's turn. A move the player selects and plays in that moment is accepted and recorded, although it was chosen on a position two moves old. If it moves the piece the lost move had already moved, its origin is now empty and both boards freeze at it; otherwise it is recorded as played, legal or not. "Play here" opens the same moment, against a position that can be several moves old if the player went on playing in the other tab. See [connection loss](../session/connection-loss.md#while-in-flight) and [a second tab](../session/second-tab.md#edge-cases).
  - *A King left in check.* After an illegal but replayable move leaves the mover's King attacked, the app offers the other player the capture of that King as a [legal destination](../glossary.md#moves-and-the-rules), with a red capture ring on the King. Playing it records a King capture, and the board freezes at that move: the banner names the honest player's own move as the one that "is not a legal move".
...
Nothing the player does can stop a bad move from being recorded, and there is nothing to cancel. A bad move that a client builds but never sends changes nothing. The player's own browser never builds one from its own legal moves, except in the two rare cases above.
...
- **The frozen-board banner appears**: a red box with white, centered text, 60 px from the top of the window and centered below the turn indicator, at most 480 px wide (and at most about half the window on a narrow one), reading "Move {N} in this game's history is not a legal move for this client (likely an app version mismatch). The board is frozen at the position before it.", where {N} is the bad move's place in the record, counting every move from 1. It has no close button and cannot be dismissed.
- **The board does not take input**, permanently. Any selection is cleared and an open promotion dialog closes; presses on pieces and cells do nothing. A held board is released from waiting, but stays frozen. The view can still be turned, except by a drag that starts on the banner, which the board never receives.
...
- **No end-game dialog**, even if the frozen position is checkmate or stalemate. A King attacked in the frozen position still glows red.
...
| Input device | No effect. The banner has no control, and the board takes no input from any device. Screen readers announce the banner when it appears. | No effect. |
...
| Pressing elsewhere or turning the view | Presses on the board do nothing. The view turns, unless the drag starts on the banner. The move list scrolls. | Normally there is nothing to press: the move arrives on the opponent's turn, or while the player's own move holds the board. The freeze lands in the view as it is, even mid-drag. |
...
**Keyboard, touch, and screen size.** The banner has no control, so there is nothing to reach with Tab; it is marked as an alert, so screen readers announce it when it appears (see [accessibility](accessibility.md)). The board takes no input from any device while frozen. The banner is 60 px from the top and up to 480 px wide, or about half the window on a narrow one, where its long text wraps onto many lines and can overlap the seat label at the top left; see [screen sizes and touch](screen-sizes-and-touch.md).
...
- **A selection or promotion dialog open when the freeze arrives** is cleared or closed, as whenever the board stops taking input.
...
- **The banner's wording.** It says "not a legal move", but replay does not check legality, and most illegal moves pass without it. "This client" is a developer's word. "(likely an app version mismatch)" suggests that updating or reloading would help, but it cannot, and when both players run the same version the cause is a modified client or one of the rare paths in [how a record breaks](#how-a-record-breaks). The banner offers no next step, though the only one is to leave, and its move number does not match the move list's row numbers. Worth a copy review.
- **Suspected bug: a stale move after a reconnect.** `client/src/screens/GameScreen.tsx:81` lets the board take input as soon as the connection is open, while the rejoin sent at `:95-101` has not yet been answered. If the player's lost move was recorded and the opponent answered during the drop, a move made in that moment is recorded against a position two moves old (or more, after "Play here"), and can freeze both boards. The same finding is recorded in [connection loss](../session/connection-loss.md#open-questions-and-verification) and [making a move](../play/making-a-move.md#open-questions-and-verification). Read from code; not reproduced. Holding the board until the snapshot arrives would close it.
- **Suspected bug: the app offers a King capture.** After an illegal move leaves a King attacked, `client/src/engine/board.ts:262-278` (legal moves) includes capturing that King, and `client/src/game/history.ts:118-121` then freezes on the capture. An honest player can thus break the record themselves and be blamed for it by the banner. Only reachable after a modified or mismatched client has already made an illegal move. Read from code; not reproduced.
- **By design, but surprising: moves after the end.** `server/modal_app.py:160-178` does not refuse moves once a game is over, and `client/src/game/history.ts:131` decides the result from the final position only, so a move recorded after checkmate takes the end-game dialog away from the other player. No test covers it.
...
- **The banner's overlap** with the seat label on narrow windows was not checked.
