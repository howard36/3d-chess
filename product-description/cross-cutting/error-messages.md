...
| "Not your turn" | `wrong_turn` | Move | The move record says it is the other seat's turn. | Rarely: a move made in the moment between a reconnect (or "Play here") and its snapshot, when the server's record is ahead of what the page shows and it is the opponent's turn there: the player's previous move was recorded but its echo was lost in the drop, or another tab moved for the player while this one was replaced. Also: a second press on the same destination within a few tens of milliseconds of the first (a very fast double press), which sends the move twice; confirmed in the scripted pass. See [making a move](../play/making-a-move.md#edge-cases). | Board screen. The snapshot arrives just before the error: it releases the held board, puts the recorded move in place, and passes the turn to the opponent. The refused move is never shown. |
