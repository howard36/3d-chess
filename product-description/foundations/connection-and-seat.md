...
- **After a mid-game drop**, the page still knows the game had started, so it stays on the board screen, showing the position as it was before the drop. The board takes input again as soon as the connection opens, a moment before the snapshot arrives. When the snapshot arrives, the board is replaced by the position it describes (with the latest move gliding in if it is one the player had not seen), and any selection is cleared.
...
- The window between a reconnect opening and its snapshot arriving, during which the board takes input against the pre-drop position, is read from code and is at most one round trip long.
