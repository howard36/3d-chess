...
That covers opening a game page from a link or bookmark, reloading it, and every reconnect after a drop or a "Play here". The rejoin is queued if the connection is not yet open. It carries the tab's client id and says whether it may [take over](#last-connection-wins) the seat from another tab: the first rejoin a page sends, and the one after "Play here", do; every later one, sent automatically after a drop, does not.
