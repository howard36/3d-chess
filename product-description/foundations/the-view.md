...
Every board screen starts with the camera in front of the board, above it and to the right, looking at the center of the cube, with the player's own slices nearest. The slices are not exactly behind one another from there, so pieces on deeper levels peek out beside the nearer ones. Both players' cameras start in the same direction; the orientation above is what makes each see their own side.

The camera stands at the distance that just fits the whole cube in the window, whatever the window's shape, with a small margin. In a window wider than it is tall, the cube's height decides the distance (about 14.5 units from the center), and there is empty space on either side; in a narrow window, such as a phone held upright, its width does, so the camera stands further back (about 21.7 units in a 375 × 667 window) and the cube is smaller, but nothing is cut off.
...
**Resizing the window** (including rotating a phone, or opening the browser's side panels) moves the camera, straight along its current line of sight, back to the distance that fits the cube in the new shape. The direction the player has orbited to, and any pan, are kept; the zoom is not.
...
**Reduced motion.** When the player's system asks for reduced motion, no move glides and no captured piece fades: the position simply changes when the move arrives, and the last-move fill still shows which two cells it involved. The setting is read each time a move lands, so changing it takes effect from the next move.
...
| Shift, Ctrl, or Cmd held | With the left button, the drag pans; with the right button, it orbits. They do not change what a press does to the board. | Pressing or releasing them mid-drag does not switch between orbit and pan; the choice is made when the drag begins. |
| Input device | Mouse: as in the table above; only a left click without a drag acts on the board. Touch: one finger orbits, two fingers pan and zoom, and only a one-finger tap acts on the board. Keyboard: no effect; the view cannot be turned from the keyboard. | Adding a second finger switches from orbit to pan-and-zoom. |
...
| A second touch point or a cancelled touch | A second finger joins the gesture as pan-and-zoom; it never acts on the board. A cancelled touch ends the gesture without acting. | A second finger switches to pan-and-zoom; a cancelled touch ends the drag where it is. |
...
**Keyboard, touch, and screen size.** The view cannot be turned from the keyboard; a keyboard player plays through the [move box](input-model.md#the-move-box) and sees the board at whatever angle it has. Touch works as described above. The scene fills the window at any size and redraws at once when the window is resized, and the camera moves back to the distance that fits the whole cube in the new shape, so a narrow window or an upright phone shows the whole cube, smaller, rather than cropping it. See [screen sizes and touch](../cross-cutting/screen-sizes-and-touch.md).

## Edge cases

- **Panned out of sight.** Pan has no limit and nothing re-centers the view, not even a resize, which keeps the pan; a player who pans the board off screen must pan it back or reload.
...
- **Occlusion.** From the default view many cells are hidden behind nearer pieces, and [a piece in front of a destination blocks it](input-model.md#what-takes-a-press). Turning the view, or typing the move in the [move box](input-model.md#the-move-box), is the only way to reach such a cell.
...
- The fitted distances (about 14.5 units in any landscape window, 21.7 in a 375 × 667 window) are computed from `client/src/three/cameraFit.ts`, whose tests check that all eight corners of the cube are in frame for wide, square, and upright windows. How much empty space the fit leaves around the cube was not judged by eye.
- A resize discarding the player's zoom is how `client/src/three/FitCameraToBoard.tsx` works; whether keeping the zoom (and only refitting when the cube would be cut off) is better is a product call.
- Reduced motion is read from `client/src/three/motion.ts` and covered by `Board.test.tsx`; not tried with a real system setting.
