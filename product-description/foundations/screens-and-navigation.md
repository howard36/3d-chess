...
The first three screens share one look: a dark page with the title "3D Chess" and a single centered element. The board screen is entirely different: a light gray-blue 3D scene with no title.
...
What survives each of these: the [stored seat](connection-and-seat.md#the-stored-seat) always survives, and the server's record of the game always survives. Everything else (the selection, the view's angle and zoom, a dismissed error, an open promotion dialog, a request that was queued or in flight) belongs to the page and is lost.
...
| Leaving the game page within the app | Resets the connection on arrival at the start screen; see the table above. | Any request in flight is abandoned by the page; the server may still record it. See each feature's own table. |
...
| The connection drops | The page stays where it is and shows its reconnecting indicator. | Same. |
...
| Another tab takes the seat | The replaced dialog appears on whichever game page screen is showing. | Same. |
...
**Connection.** One connection per tab, opened when the app loads and kept across the in-app move from the start screen to a new game; reset on every arrival at the start screen; replaced by a new one on every full page load.
...
**Keyboard, touch, and screen size.** Browser Back and Forward work from the keyboard as usual. Nothing on the pages changes with window size except layout; see [screen sizes and touch](../cross-cutting/screen-sizes-and-touch.md).
...
- The phase rules are covered by `client/src/App.test.tsx`; the reset on returning to the start screen by `client/e2e/gameOver.spec.ts`; the crash screen by `client/src/components/ErrorBoundary.test.tsx`. Which real failures reach the crash screen is unconfirmed.
