...
| Connection state | The banner does not depend on it. It stays up, with its text, while connecting or reconnecting, alongside the "Reconnecting…" box at the top right. Replaced: the replaced dialog is drawn over the banner, which stays behind it, darkened. On the board screen it is then inert: "✕" can be neither clicked nor reached with Tab. On the share-link, join, and joined screens it is only covered: Tab still reaches "✕". | A drop or a reconnect neither shows nor hides it; after "Play here" it is still up. |
...
| Another tab takes the seat | The replaced dialog covers the banner. On the board screen "✕" can be neither clicked nor reached with Tab until the dialog goes; on the pre-game screens Tab still reaches it. After "Play here" the banner is still up. The newer tab has its own errors and starts with no banner; errors are never shared between tabs. | This tab's connection is closed, so an answer that had not arrived is lost. |
...
- The seat-in-use refusal is left out of the banner by `client/src/screens/GameScreen.tsx:71-76` and answered with the replaced dialog (`:81-83` and `:261`). Covered by "GameScreen does not take the seat back after a reconnect, and offers Play here" in `client/src/App.test.tsx`.
