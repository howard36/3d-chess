...
- **A stale position after reconnecting.** Between a new connection opening and the rejoin's snapshot arriving, the indicator reflects the position from before the drop. If the player's last move had in fact been recorded, it still names the player, and a move sent in that window is refused with "Error: Not your turn"; the snapshot then changes the indicator. If the opponent had also answered that move, the indicator is right by accident, and a move sent in that window is accepted against a position two moves old. See [making a move](../play/making-a-move.md#edge-cases) and [connection loss](../session/connection-loss.md#while-in-flight).
